...
const SYSTEM_PROMPT =
  ...
  "5. Be concise but include all necessary context\n" +
  "Format your response as a prompt the user can send to start the new thread.\n" +
  "Do not include any preamble like 'Here is the prompt' - just output the prompt itself.";

export default function (pi: ExtensionAPI) {
  pi.registerCommand("handoff", {
    description:
      "Transfer context to a new focused session. Usage: /handoff <goal for new thread>",
    handler: async (args, ctx) => {
      ...
      const editedPrompt = await ctx.ui.editor(
        "Edit handoff prompt (enter to submit, esc to cancel)",
        result,
      );
      ...
      ctx.ui.notify(
        "Handoff ready. Add context and submit when ready.",
        "info",
      );
    },
  });
